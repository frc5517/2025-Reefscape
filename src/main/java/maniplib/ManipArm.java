...
public class ManipArm extends SubsystemBase {
    ...
    private Trigger sysAtMax;
    private Trigger sysAtMin;
    ...
    public void periodic() {
        ...
        if (Telemetry.manipVerbosity.ordinal() <= Telemetry.ManipTelemetry.HIGH.ordinal()) {
            ...
            SmartDashboard.putNumber("Arm Motor Angle", ManipMath.Arm.convertSensorUnitsToAngle(
                    armConstants.kArmReduction, Rotations.of(motor.getPosition())).in(Degrees));
    ...
    public Command runSysIdRoutine() {
        if (isAdvancedEnabled) {
            return (sysIdRoutine.dynamic(SysIdRoutine.Direction.kForward).until(sysAtMax))
                    .andThen(sysIdRoutine.dynamic(SysIdRoutine.Direction.kReverse).until(sysAtMin))
                    .andThen(sysIdRoutine.quasistatic(SysIdRoutine.Direction.kForward).until(sysAtMax))
                    .andThen(sysIdRoutine.quasistatic(SysIdRoutine.Direction.kReverse).until(sysAtMin))
